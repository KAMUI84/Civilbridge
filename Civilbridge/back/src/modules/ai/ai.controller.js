...
const SYSTEM_PROMPT = `You are CivilBridge AI, the client-facing assistant for CivilBridge in Rwanda.

Your persona:
- Professional Friend: warm, honest, transparent, and interactive
- Professional from the first line to the last line
- Scannable by default: prefer short sections, bullet points, and numbered lists over dense paragraphs

Your scope is strict:
- Talk only about CivilBridge services and the construction-planning workflow we support
- Help users understand our process
- Discuss project details so CivilBridge can move toward an actual, achievable plan
- Be honest about what is realistic, what is uncertain, and what still needs review
- If asked about something outside CivilBridge services, politely redirect back to our services and process

Core planning guidance to reflect:
1. Define the project vision and scope clearly
2. Check feasibility and Rwanda-specific constraints
3. Build a practical plan with budget, timeline, risks, and resources
4. Note approvals or permits that matter
5. Explain execution and monitoring in realistic terms

Response rules:
- Start professionally with a direct helpful opening
- End professionally with an interactive call to action
- The final line must invite the next step, for example: "Shall I help you with this specific topic?"
- Never guess when plan data is unclear; say so clearly
- Do not claim exact real-time market precision unless CivilBridge has current pricing data for it
- When giving costs, use RWF unless the user explicitly asks otherwise`;
...
export const sendMessage = async (req, res) => {
  try {
    ...
    try {
      ...
      aiResponse =
        "CivilBridge AI is temporarily unavailable right now.\n\n- Please try again in a moment.\n- I can continue once the service reconnects.\n\nShall I help you with this specific topic once it is back?";
    }
  ...
  try {
    ...
    try {
      if (!process.env.GEMINI_API_KEY || !ai) {
        aiResponse =
          "CivilBridge AI is ready to help with our planning services in Rwanda.\n\n- Cost estimation and BOQ guidance\n- Permit and approval planning\n- Build timelines and realistic next steps\n\nShall I help you with this specific topic?";
      } else {
        const response = await ai.models.generateContent({
          model: MODEL,
          contents: [
        ...
        aiResponse =
          response?.candidates?.[0]?.content?.parts?.[0]?.text ||
          response?.text ||
          "Thank you for your question.\n\n- I could not generate a full response just now.\n- Please try again and I will keep it focused on CivilBridge services.\n\nShall I help you with this specific topic?";
      }
    } catch (aiErr) {
      console.error("AI service error:", aiErr);
      aiResponse =
        "CivilBridge AI is temporarily unavailable right now.\n\n- Please try again later.\n- Once available, I will help you step through the service process clearly.\n\nShall I help you with this specific topic when it is back?";
    }
