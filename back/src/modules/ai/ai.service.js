...
function mapBoqExtractionResult(data, context) {
  ...
  return {
    ...
    risks: data.risks || [],
    confidence: {
      overall: toNumber(data.confidence?.overall),
      notes: data.confidence?.notes || "",
    },
  };
}

function mapPlanGenerationResult(data) {
  const boq = mapBoqItems(data.boq);
...
function mapConversationalEstimationResult(data) {
  return {
...
export async function analyzePlanDocument({
  ...
  try {
    return await runStructuredTask(AI_TASKS.BOQ_EXTRACTION, params, {
    ...
    return runStructuredTask(AI_TASKS.BOQ_EXTRACTION, params, {
