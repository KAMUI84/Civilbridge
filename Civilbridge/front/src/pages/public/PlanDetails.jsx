      ...
      {relatedPlans.length ? (
        <div style={{ marginTop: 40 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16, marginBottom: 16, flexWrap: "wrap" }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 24, fontWeight: 800 }}>Similar to This</h2>
              <p style={{ margin: "8px 0 0", color: "#64748b", fontSize: 14 }}>
                Recommended plans keep users browsing after one detail page.
              </p>
            </div>
            <Link to="/plans" style={{ color: "#3b82f6", fontWeight: 700, textDecoration: "none", fontSize: 14 }}>
              View all plans →
            </Link>
          </div>
            ...
            {relatedPlans.map((item) => (
              ...
              >
                ...
                <div style={{ padding: 16 }}>
                  ...
                  <div style={{ fontSize: 13, color: "#64748b", marginBottom: 8 }}>{item.format} • {item.sheets} sheets</div>
...
export default function PlanDetails() {
    ...
    try {
      ...
      const response = await plansService.requestFollowUp(id, {
        ...
        notes: [
          ...
          requestNotes ? `Reason: ${requestNotes}` : "",
        ].filter(Boolean).join("\n"),
      });
      setRequestState({
        saving: false,
  ...
  const relatedPlans = getRelatedPlanLibraryItems(id, plan.category);
              ...
              {!isAuthed ? (
                ...
                <form onSubmit={handleFollowUpRequest} style={{ display: "grid", gap: 12 }}>
                  ...
                  <textarea
                    ...
                    rows={4}
                    placeholder="Reason for follow-up: what do you need next from this plan?"
