...
import { getDashboardConfig, getVisibleNavigation } from './utils/dashboardConfig';
...
export default function DashboardLayout() {
  ...
  const visibleNavigation = React.useMemo(
    () => getVisibleNavigation(user?.role, location.pathname),
    [location.pathname, user?.role],
  );
  const layoutConfig = React.useMemo(
    () => ({ ...dashboardConfig, navigation: visibleNavigation }),
    [dashboardConfig, visibleNavigation],
  );
    ...
    >
      ...
      {rightPanelOpen ? (
        <>
          ...
          <RightPanel onClose={() => setRightPanelOpen(false)} user={user} config={layoutConfig} />
        </>
      ) : null}
    </div>
  );
